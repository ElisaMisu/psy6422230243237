...
After consulting the ONS metadata I discovered that '[c]' was in the column if the value was less than 10 so it was suppressed. To correct this I created a cleaned dataframe where all cells not including [c] were retained while those including [c] were excluded. Following that I renamed the variables, see code below.
...
-   *age == "All ages 16 years and over"* ensures that only
    rows where the age column exactly matches "All ages 16 years and
    over" are retained, excluding the columns breaking the data down by age group.
